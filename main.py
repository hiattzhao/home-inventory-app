...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            '', 'Purchase Date', 'Name', 'Category', 'Notes', 'Photo', 'Value'
        ])
        ...
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)  # Photo
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents)  # Value
    ...
    def load_items(self, items=None):
        ...
        for row, item in enumerate(items):
            ...
            # Notes (column 4)
            self.table.setItem(row, 4, QTableWidgetItem(item[5] or ""))
            
            # Photo count (column 5)
            photo_count = len(self.db.get_item_photos(item[0]))
            if photo_count == 0 and item[6]:  # Fallback for old single photo
                photo_count = 1
            photo_text = f"{photo_count} photo{'s' if photo_count != 1 else ''}" if photo_count > 0 else "No photos"
            self.table.setItem(row, 5, SortableTableWidgetItem(photo_text, photo_count))
            
            # Value (column 6)
            value_float = float(item[3])
            self.table.setItem(row, 6, SortableTableWidgetItem(f"${value_float:,.2f}", value_float))
    ...
    def edit_item(self):
        ...
        categories = self.db.get_categories()  # Refresh categories from database
        dialog = ItemDialog(self, categories, item_data)
